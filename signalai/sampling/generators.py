...
from vibdata.deep.DeepDataset import DeepDataset, resample_dataset, convertDataset
...
class FoldIdxGeneratorUnbiased:
    def __init__(self, dataset: DeepDataset,
        ...
        self.multiround = multiround if multiround is not None else getattr(custom_group_dataset, 'MULTIROUND', False)
        self.dataset_name = dataset_name if dataset_name else "default_dataset"

        self.class_def = class_def if class_def is not None else getattr(custom_group_dataset, 'CLASS_DEF', {})
    ...
    def generate_folds(self):
        ...
        # Flatten all fold arrays into a single numpy array
        flat_folds = np.concatenate([np.ravel(f) for f in folds])

        # Verify all expected folds exist
        unique_folds = np.unique(flat_folds)

        expected_folds = np.arange(unique_folds.max() + 1)

        if not np.array_equal(unique_folds, expected_folds):
            raise ValueError(
