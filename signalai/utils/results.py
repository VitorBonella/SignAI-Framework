...
class ExperimentResults:
    """Stores all results for an experiment."""
    experiment_name: str
    description: str
    model_name: str
    ...
    def show_results(self, figsize: tuple = (10, 6), cmap: str = 'Blues'):
        """Displays experiment results in table and confusion matrix format."""
        ...
        metrics_data = {
            'Metric': ['Mean Accuracy', 'Std Accuracy', 'Mean F1', 'Std F1'],
            'Value': [
        ...
        if self.folds:
            ...
            for i, fold in enumerate(self.folds):
                fold_data.append({
                    'Fold': i + 1,
                    'Accuracy': f"{fold.metrics.get('accuracy', 0):.4f}",
    ...
    def _plot_confusion_matrix(self, cm: np.ndarray, figsize: tuple = (10, 6), cmap: str = 'Blues'):
        ...
        ax = sns.heatmap(cm, annot=True, fmt='d', cmap=cmap, cbar=True, square=True, linewidths=0.5, linecolor='white')
        
        for i in range(cm.shape[0]):
            for j in range(cm.shape[1]):
                if cm[i, j] > 0:
                    ax.text(j + 0.5, i + 0.3, f'{cm_percentage[i, j]:.1f}%', ha='center', va='center', fontsize=10, color='red')
